...
const Select = ({
  ...
  const [value, setValue] = useState(null); // Modification ici, initialisez la valeur à null
  const [collapsed, setCollapsed] = useState(true);

  const changeValue = (newValue) => {
    onChange(newValue); // Modification ici, passez la nouvelle valeur à la fonction onChange
    setValue(newValue);
    setCollapsed(true);
  };
